...
x_test_raw , y_test_raw  = read_data(sys.argv[1])
x_test     ,  y_test     = make_batches(x_test_raw,  y_test_raw )


transformer.load_weights(sys.argv[2])

for i in range(0, 100):
    result = transformer((x_test[0][i : i + 1], x_test[1][i : i + 1]))
    print("The instruction {0} was guessed to be {1} and was actually {2}.".format(x_test[1][i : i + 1], result, y_test[i : i + 1]))